...
export function useMusicPlayer(options: UseMusicPlayerOptions) {
  ...
  const playModeIndex = ref<0 | 1 | 2>(playModeToIndex(options.initialPlayMode ?? 'list'))
  ...
  const volume = ref(
    import.meta.client && localStorage.getItem('music-player-volume') !== null
      ? Number.parseFloat(localStorage.getItem('music-player-volume')!)
      : (options.initialVolume ?? 0.7),
  )
  ...
  const currentTrack = computed(() => {
    const list = options.tracks.value
  ...
  onMounted(() => {
    applyVolume()
    if (options.tracks.value.length > 0) {
      const startIndex = playModeIndex.value === 2
        ? Math.floor(Math.random() * options.tracks.value.length)
        : 0
